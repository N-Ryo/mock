class User < ApplicationRecord
  ...
  enum proficiencies: { beginner: 1, intermediate: 2, advanced: 3, elite: 4, hero: 5 }
  ...
  # カテゴリをフォローする
  def category_follow(category)
    categories << category
  end

  # カテゴリをフォロー解除する
  def category_unfollow(category)
    user_category_relationships.find_by(category_id: category.id).destroy
  end

  # 現在のユーザーがカテゴリをフォローしてたらtrueを返す
  def category_following?(category)
